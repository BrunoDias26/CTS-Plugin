...
def get_rightmost_tag(tags, view):
    """Retorna a tag mais à direita (maior projeção na direção RightDirection)."""
    rightmost = None
    max_proj = float("-inf")
...
def align_tags(reference_tag, tag_list, view, spacing=0.15):
    """Alinha tags verticalmente, mantendo o X da referência (direita)."""
    ...
    t = Transaction(doc, "Right alignment")
    t.Start()
