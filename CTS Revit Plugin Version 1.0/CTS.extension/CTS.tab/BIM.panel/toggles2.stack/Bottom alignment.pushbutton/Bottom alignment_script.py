...
def get_bottommost_tag(tags, view):
    """Retorna a tag mais baixa na vista (menor projeção na direção UpDirection)."""
    bottommost = None
    min_proj = float("inf")
...
def align_tags(reference_tag, tag_list, view, spacing=0.15):
    """Alinha tags horizontalmente, mantendo o Y da referência (inferior) e espaçamento fixo."""
    ...
    t = Transaction(doc, "Bottom alignment")
    t.Start()
...
if active_view.ViewType in allowed_views:
    ...
    tags = select_tags()
    if tags:
        ref_tag = get_bottommost_tag(tags, active_view)
        if ref_tag:
